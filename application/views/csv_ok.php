...
$data = explode("ABUHANIF", $_GET['pay']);
...
echo $FROM_REK.",".$FORM_NAME.",IDR,".(round($TotalBayar,0)-($MATERAI*$JumlahSupplier)).",".$NotePad.",".$JumlahSupplier.",".$TANGGAL_JT.",\n";
...
while ($r = $hasil->fetch()) {
	...
	echo "".$r['SUPPLIER_ACC'].",".str_replace(",", ".", $r['SUPPLIER_ACCNAME']).",IDR,".(round($r['AMOUNT'],0)-$MATERAI).",".$PmtNmbr.",".$r['BranchName'].",Y,Y"."\n";
}
