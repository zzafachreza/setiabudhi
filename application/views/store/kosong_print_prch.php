		...
		<tbody>
			...
			<tr>
				<td style="text-align: center;"><?php echo $no; ?> <span style="width: 10px;height: 30px; background-color: <?php echo $warnaPO ?>">&nbsp;&nbsp;&nbsp;</span> </td>
					...
					<td><input id="ket<?php echo $no ?>" class="form-control" placeholder="masukan keterangan" type="text" name="keterangan" value="<?php echo $r['keterangan']; ?>" data-id="<?php echo $r['id']; ?>"></td>
					<td>
						<button id="btn<?php echo $no ?>" onClick="save('<?php echo $no ?>')" class="btn btn-danger">Simpan</button>
					</td>
...
<script type="text/javascript">
	function save(no){
		...
		var id = $("#ket"+no).attr("data-id");
		var keterangan = $("#ket"+no).val();
		$.ajax({
			url:'kosong_update_keterangan',
			type:'POST',
			data:{
				id:id,
